...
urlpatterns = [
    ...
    url(
        r'^project/(?P<project_id>[0-9]+)/sample/add_bulk/$',
        views.SampleBulkAddView.as_view(),
        name="sample_add_bulk"
    ),
    ...
    url(
        r'^project/(?P<project_id>[0-9]+)/user/(?P<pk>[0-9]+)$',
    ...
    # ajax views
    url(r'^(?P<model>[A-Za-z]+)/select2/$', views.LimsSelect2Ajax.as_view(), name='ajax_select2'),
